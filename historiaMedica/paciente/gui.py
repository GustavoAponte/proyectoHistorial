...
class Frame(tk.Frame):
    ...
    def camposPaciente(self):
        ...
        self.entryFecNacimiento = DateEntry(self, textvariable=self.svFecNacimiento, width=12, locale ='es_US',background='darkblue', foreground='white',borderwidth=2)
        
        #self.entryFecNacimiento = tk.Entry(self, textvariable=self.svFecNacimiento)
        
        self.entryFecNacimiento.config(width=27, font=('ARIAL',15))
        self.entryFecNacimiento.grid(column=1, row=4, padx=10, pady=5, columnspan=1)
        
        self.svEdad = tk.StringVar()
        #self.svEdad.set(self.calcularEdad)
        self.entryEdad = tk.Entry(self, textvariable=self.svEdad)
        
        #print("svedad",type(self.svEdad))
        self.entryEdad.config(width=50, font=('ARIAL',15),state="readonly")
        ...
        self.btnEdad = tk.Button(self, text='Calcular Edad', command = self.calcularEdad)
        self.btnEdad.config(width=12, font=('ARIAL',12,'bold'), fg='#DAD5D6', 
                                bg='#53005B', cursor='hand2',activebackground='#C774CF')
        self.btnEdad.grid(column=3,row=4, padx=10, pady=5, columnspan=1)
    ...
    def calcularEdad(self, *args):
        #poner la fecha de nacimiento de str a datetime
        self.date1 = self.svFecNacimiento.get()
        #print("self.date1",self.date1)
        #print("date1 type",type(self.date1))

        self.date1 = self.date1.replace('/','-')
        fecnac = datetime.strptime(self.date1, "%d-%m-%Y")
        #print("self.fecnac",fecnac)
        #print("fecnac type",type(fecnac))

        #poner la fecha de hoy en datetime
        dia_hoy = date.today()
        fecha_fin = dia_hoy.strftime('%d-%m-%Y')
        fecha_fin = datetime.strptime(fecha_fin, '%d-%m-%Y')

        #calcular la edad por diferencia
        tiempo_transc = relativedelta.relativedelta(fecha_fin, fecnac)
        #print('Tiempo transcurrido: ',tiempo_transc.years,'años,',
        #    tiempo_transc.months,'meses,',tiempo_transc.days,'dias')

        #actualizar la edad        
        self.svEdad.set(tiempo_transc.years)
        #print(type(tiempo_transc.years)) #int
        #print(type(self.svEdad)) #Tkinter Stringvar  
    ...
    def habilitar(self):
        ...
        self.btnEdad.config(state='normal')
    ...
    def deshabilitar(self):
        ...
        self.btnEdad.config(state='disabled')
